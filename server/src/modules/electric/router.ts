...
export const electricRouter = Router();
electricRouter.use(requireAuth);

electricRouter.get(
  "/",
  ah(async (_req, res) => {
    const items = await prisma.electricMeterReading.findMany({
...
electricRouter.post(
  ...
  ah(async (req, res) => {
    ...
    const rate =
      body.ratePerUnit ??
      Number((await prisma.setting.findUnique({ where: { key: "ELECTRIC_RATE" } }))?.value ?? 7);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (body.createInvoice && totalAmount > 0) {
        ...
        const count = await tx.invoice.count({
          where: { invoiceNo: { startsWith: `ELEC-${body.period}` } },
        });
        const inv = await tx.invoice.create({
          data: {
            invoiceNo: `ELEC-${body.period}-${String(count + 1).padStart(5, "0")}`,
            contractId: contract?.id,
            type: InvoiceType.ELECTRIC,
            period: body.period,
            ...
            items: {
              create: [
                {
                  description: `ค่าไฟ ${body.period} (${unitsUsed} หน่วย × ${rate})`,
                  quantity: unitsUsed,
                  unitPrice: rate,
                  amount: totalAmount,
                },
      ...
      return tx.electricMeterReading.create({
        data: {
          lockId: body.lockId,
