            ...
            {projectsQ.data?.map((p, idx) => {
              ...
              return (
                <tr key={p.id} className="border-t hover:bg-slate-50">
                  ...
                  <td className={`p-3 text-right font-semibold ${p.remaining < 1 ? "text-green-600" : "text-red-600"}`}>
                    {fmt(p.remaining)}
                  </td>
                  <td className={`p-3 text-right ${p.remaining < 1 ? "text-green-600" : "text-red-600"}`}>
